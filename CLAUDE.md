...
RAG 驱动的 MCP 服务器，为 GrapeCity 产品文档（SpreadJS、GcExcel、Forguncy、Wyn）提供向量检索。单 Docker 容器内跑 Node.js + Qdrant，通过 HTTP StreamableHTTP 协议对外暴露 MCP 工具。
...
```
├── src/                    # 主源码
│   ├── index.ts            # 入口：加载配置→创建 embedder/searcher→启动 HTTP
│   ├── embed.ts            # CLI 嵌入脚本：加载文档→分块→索引到 Qdrant
│   ├── server.ts           # Express 服务：session 管理、多产品 MCP 端点、健康检查
│   ├── config/             # 配置层
...
│   │   ├── qdrant-client.ts# Qdrant REST 封装：collection 管理、混合/稠密查询、RRF
...
│   │   ├── cn.yaml         # 中文变体：collection 名、raw_data 路径、CDN/npm 资源
│   │   └── en.yaml         # 英文变体
│   ├── gcexcel/
│   │   ├── product.yaml
│   │   └── cn.yaml
...
├── storage/                # 运行时持久化数据
│   ├── checkpoints/        # 索引断点，支持续传
│   ├── logs/               # 运行日志
│   └── qdrant/             # Qdrant 向量存储
├── frontend/               # 前端（React + Vite SPA + TypeScript），内嵌于 Express 静态服务
│   └── src/
│       ├── App.tsx         # 路由壳 + ThemeProvider
│       ├── main.tsx        # 入口
│       ├── pages/          # 页面组件（SetupGuide, Playground, Landing, Profile）
│       ├── components/     # 共享组件（Header, CodeBlock, MarkdownRenderer 等）
│       ├── hooks/          # useTheme, useMcpSession, useClipboard
│       ├── lib/            # mcp-client, config, markdown
│       ├── styles/         # theme.css, reset.css, markdown.css, fonts.css
│       └── i18n/           # 国际化（zh/en/ja）
├── benchmark/              # 测试与基准
│   ├── protocol/           # MCP 协议合规测试（vitest）
│   ├── retrieval/          # 检索质量测试
│   │   ├── datasets/       # 各产品测试查询集（YAML）
│   │   └── call-search.js  # 调用 search 工具的辅助脚本
│   └── promptfooconfig.yaml# PromptFoo 集成配置
├── .github/workflows/
│   └── deploy.yml          # CI/CD 部署
├── Dockerfile              # 多阶段构建：build TS → 合并 Qdrant → slim runtime
├── docker-compose.yml      # 服务定义，卷挂载 raw_data + storage
├── docker-entrypoint.sh    # 容器入口：启动 Qdrant → 等待就绪 → 启动 Node
├── embed.sh                # 宿主机便捷脚本：docker exec 进容器执行嵌入
├── .env.example            # 环境变量模板
├── package.json            # 依赖与脚本
...
### 整体架构（`src/server.ts` + `src/mcp/`）

HTTP 层和 MCP 协议层分离。Express 负责路由和 session 管理，MCP SDK 负责协议序列化。

**多产品端点**：每个产品注册独立路由 `POST /mcp/{productId}`，拥有独立的 session 池。启动时 `src/index.ts` 并行加载所有产品配置和 searcher，传入 `startServer()`。

**Session 生命周期**（`src/server.ts:65-166`）：
...
**每次请求注入 RequestContext**（`src/shared/request-context.ts`）：通过 `AsyncLocalStorage.run()` 将 requestId / sessionId / productId / clientInfo 传递到 MCP tool handler 内部，无需参数穿透。

### MCP Server 封装（`src/mcp/server.ts`）

`MCPServer` 类包装 `@modelcontextprotocol/sdk` 的 `McpServer`。构造时：
- 调用 `buildInstructions(config)`（`src/mcp/instructions.ts`）生成 system prompt，由通用工作流说明 + product.yaml 的 `instructions` 字段拼接
...
### 基类能力（`src/document/chunkers/base.ts`）
...
**MarkdownChunker**（`src/document/chunkers/markdown.ts`）：
- 小文件（≤ chunkSize）直接输出
- h2 主切 → 仍超限则 h3 二次切 → `splitProtected` 兜底
- 续块前缀父级 header 保持上下文
- 每个 chunk 写入 `section_path` 元数据（如 `["安装", "npm 安装"]`）

**TypeDocChunker**（`src/document/chunkers/typedoc.ts`）：
...
**JavaDocChunker**（`src/document/chunkers/javadoc.ts`）：
按 `doc.metadata.category` 分三种策略：
...
每个 Chunk 携带的 metadata（定义在 `src/document/types.ts`）：
...
### Qdrant Collection Schema（`src/rag/qdrant-client.ts:61-77`）
...
5. 每批成功后写 checkpoint 到 `storage/checkpoints/`
6. 全部完成后清除 checkpoint 文件

### 检索流程（`src/rag/searcher.ts`）
...
### Embedding（`src/rag/embedder.ts`）

`VoyageEmbedder` 关键设计：
- **动态批次**：按 token 数估算（中文 ÷ 1.5 字符/token，英文 ÷ 4），单批不超过 Voyage 100k token 上限
- **限流集成**：每批调用前检查 `RateLimiter`（滑动窗口 RPM/TPM），超限等待 retryAfter
...
npm run embed         # 本地嵌入（需 .env 配置）
npm run typecheck     # 类型检查
npm run test          # vitest
./embed.sh --force    # 容器内全量重建
...
2. 原始文档放入 `raw_data/{lang}/{productId}/{category}/`
3. 环境变量 `PRODUCT` 中追加产品 ID
4. 执行嵌入：`./embed.sh --product {productId}`
...
| `DOC_LANG` | 语言变体（cn/en/ja） |
| `VOYAGE_API_KEY` | Voyage AI API 密钥 |

其余可选变量见 `.env.example` 和 `src/config/env.ts`。
